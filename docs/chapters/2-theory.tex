...
\begin{itemize}
      ...
      \item \textbf{\ac{RoPE}}: Essa codificação aplica uma rotação no ângulo \begin{math}m\theta\end{math} nos pares de vetores de
            \textit{query} e \textit{key}\footnote{Esses vetores serão explicados na seção \autoref{sec:transformers}}, sendo que \begin{math}\theta\end{math} é um ângulo
...
\subsubsubsection{LoRa} % TODO: Expandir a seção

Essa técnica se consiste na introdução de matrizes treináveis menores ao \ac{MLLM}, deixando as matrizes originais intactas durante o treinamento. Assim que treinadas,
essas matrizes são adicionadas às originais \cite{hu2021lora}.

\ac{LoRa} reduz drasticamente o custo do treinamento comparado ao \textit{fine-tuning} completo, usando, por exemplo, três vezes menos memória e em dez mil vezes menos
parâmetros durante o \textit{fine-tuning} do modelo \ac{GPT}-3 \cite{llm_survey_2023}.

\subsubsubsection{QLoRa} % TODO: Expandir a seção
...
utiliza este conceito é o \ac{QLoRa}, que segundo \textcite{qlora}, foi usada no \textit{fine-tuning} de um modelo que atingiu 99.3\% do desempenho do \textit{ChatGPT}
